...
export function centerSelection(doc: IconDoc, addrs: readonly Address[]): Command {
  ...
  if (targets.length === 0) return command('Center', () => {});

  let box: Box = EMPTY_BOX;
  for (const el of targets) box = union(box, geometricBox(el));
  if (isEmpty(box)) box = documentGeometricBox(doc);
  if (isEmpty(box)) return command('Center', () => {});

  const c = center(box);
  // Quantise the move to the half grid. An exact centering delta is an arbitrary
  ...
  // center is invisible; scattering the geometry off the grid is not.
  const half = (v: number): number => Math.round(v * 2) / 2;
  const d = { x: half(CANVAS / 2 - c.x), y: half(CANVAS / 2 - c.y) };

  return command(ids.size > 0 ? 'Center selection' : 'Center icon', (draft) => {
